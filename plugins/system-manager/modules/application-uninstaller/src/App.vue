...
<template>
  <main class="shell">
    <header class="topbar">
      ...
      <button class="icon-button" :disabled="loading" title="重新扫描" @click="refresh"><RefreshCw :size="18" :class="{ spin: loading }" /></button>
    </header>

    <div v-if="error" class="alert error"><AlertTriangle :size="17" /><span>{{ error }}</span><button @click="error = ''"><X :size="15" /></button></div>
    ...
    <section v-if="!plan" class="catalog">
      ...
      <label class="search"><Search :size="17" /><input v-model="query" placeholder="搜索名称、发布者或版本" /></label>
      ...
      <div v-else-if="visibleApps.length" class="app-list">
        <button v-for="app in visibleApps" :key="app.id" class="app-row" @click="chooseApp(app)">
      ...
      <div v-else-if="scan" class="empty"><Search :size="28" /><p>没有匹配的应用</p></div>
    </section>

    <section v-else class="plan">
      <button class="back" @click="back"><ArrowLeft :size="16" />返回应用列表</button>
      <div class="plan-head">
      ...
      <div v-else class="candidate-list">
        <article v-for="item in plan.candidates" :key="item.id" class="candidate" :class="{ disabled: !item.deletable }">
          <button class="checkbox" :class="{ checked: selectedIds.includes(item.id) }" :disabled="!item.deletable" @click="toggle(item.id, item.deletable)"><Check v-if="selectedIds.includes(item.id)" :size="14" /></button>
      ...
      <div class="confirm-box">
        <label>输入 <strong>{{ plan.app.name }}</strong> 确认</label>
        <input v-model="confirmation" :placeholder="plan.app.name" autocomplete="off" />
    ...
    <div v-if="result && resultSummary" class="modal-backdrop">
      <div class="modal"><div class="result-icon"><Check :size="26" /></div><h2>处理完成</h2><p>移入废纸篓 {{ resultSummary.trashed }} 项，卸载器 {{ resultSummary.launched }} 项，失败 {{ resultSummary.failed }} 项。</p><div v-if="resultSummary.failed" class="result-errors"><p v-for="item in result.results.filter(i => i.status === 'failed')" :key="item.candidateId">{{ item.message }}</p></div><button class="primary-button" @click="result = null; refresh()">返回并重新扫描</button></div>
    </div>
  </main>
</template>
