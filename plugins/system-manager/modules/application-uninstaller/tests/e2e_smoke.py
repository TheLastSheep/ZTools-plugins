...
window.applicationUninstaller = {
  ...
  async executePlan(request) { if (request.confirmation !== 'Acme Editor') throw new Error('bad confirmation'); return { planId:request.planId,completedAt:new Date().toISOString(),results:request.selectedIds.map(candidateId => ({candidateId,status:'trashed'}))}; },
...
def exercise(page, screenshot=False):
    ...
    confirm = page.get_by_placeholder("Acme Editor")
