...
<template>
  <div class="editor-backdrop" @click.self="emit('close')">
    <section class="editor glass-surface" aria-labelledby="editor-title">
      <header><div><p>PasteboardPro Editor</p><h2 id="editor-title">{{ mode === 'create' ? '新建文本' : mode === 'rename' ? '重命名' : '编辑文本' }}</h2></div><button type="button" aria-label="关闭编辑器" @click="emit('close')">×</button></header>
