...
<nav class="strip" aria-label="Pinboards">
  ...
  {#each pinboards as pinboard (pinboard.id)}
    <div role="listitem" class:active={activeId === pinboard.id} class:managing={managingId === pinboard.id} class="chip" oncontextmenu={(event) => { event.preventDefault(); managingId = managingId === pinboard.id ? undefined : pinboard.id; }} ondragover={(event) => event.preventDefault()} ondrop={(event) => { event.preventDefault(); drop(event, pinboard.id); }}>
      {#if editingId === pinboard.id}
        <input aria-label="重命名 Pinboard" bind:value={draft} onkeydown={(event) => { if (event.key === "Enter") commitRename(pinboard.id); if (event.key === "Escape") editingId = undefined; }} onblur={() => commitRename(pinboard.id)} />
      {:else}
        <button type="button" onclick={() => onselect(pinboard.id)} ondblclick={() => { editingId = pinboard.id; draft = pinboard.name; }}><span class="dot" style:background={pinboard.color}></span>{pinboard.name}</button>
      ...
      {#if managingId === pinboard.id}
        <div class="controls" aria-label={`${pinboard.name} 管理选项`}>
          <input class="color" type="color" value={pinboard.color} aria-label="Pinboard 颜色" onchange={(event) => onupdatecolor(pinboard.id, event.currentTarget.value)} />
          <button type="button" aria-label="向左移动" disabled={pinboards[0]?.id === pinboard.id} onclick={() => onmove(pinboard.id, -1)}>←</button>
          <button type="button" aria-label="向右移动" disabled={pinboards.at(-1)?.id === pinboard.id} onclick={() => onmove(pinboard.id, 1)}>→</button>
  ...
  {/each}
  {#if creating}
    <input class="new" aria-label="新建 Pinboard" placeholder="Pinboard 名称" bind:value={draft} onkeydown={(event) => { if (event.key === "Enter") commitCreate(); if (event.key === "Escape") creating = false; }} onblur={commitCreate} />
  {:else}
    <button class="add" type="button" aria-label="新建 Pinboard" onclick={() => { creating = true; draft = ""; }}>＋</button>
  {/if}
  <span class="hint">⌘1–9 Quick Paste</span>
</nav>
...
<style>
  ...
  .manage{width:22px;min-width:22px;padding:0;overflow:hidden;opacity:0;transition:opacity 120ms ease}.chip:hover .manage,.chip:focus-within .manage,.chip.managing .manage{opacity:1}.controls{display:flex;gap:3px;align-items:center;padding:2px 4px 2px 1px}.controls button{min-width:23px;min-height:23px;padding:0 5px}.controls button:disabled{cursor:default;opacity:.3}.controls .danger{color:#d94b57}.color{width:23px;height:23px;padding:2px;border-color:var(--pb-line);cursor:pointer}
